...
map(iris_nested$data, ~ .x$Sepal.Length)
iris_nested$data %>% map(~ .x$Sepal.Length)
...
#como la seleccionamos en el dataset anidado
map(iris_nested$data, ~ mean(.x$Sepal.Length))
...
iris_nested$data %>% map(~ mean(.x$Sepal.Length))

iris_nested %>% 
  mutate(sep_len_mean = map_dbl(data, ~ mean(.x$Sepal.Length)))

#tambien podemos crear nuevos columnas-listas 

iris_nested <- iris_nested %>% 
  mutate(sep_len_mean = map_dbl(data, ~ mean(.x$Sepal.Length)),
         sep_len_double = map(data, ~ (.x$Sepal.Length * 2)))
...
#para modelar Petal.Length a partir de Sepal.Length
...
iris_mod_tidy %>% filter(term == "Sepal.Length") %>% select(estimate)
...
iris_nested <- iris_nested %>%
  mutate(mod = map(data, ~lm(Petal.Length ~ Sepal.Length, data = .x)))
...
#como lo hariamos uno a uno
iris_nested$mod[[1]] %>% tidy() %>% filter(term == "Sepal.Length") %>% .$estimate
...
iris_nested <- iris_nested %>%
  mutate(mod = map(data, ~lm(Petal.Length ~ Sepal.Length, data = .x)),
         ...
         coef = map_dbl(mod_tidy, ~ filter(.x, term == "Sepal.Length") %>% .$estimate))
